document.addEventListener("DOMContentLoaded", function () {
    d3.csv("data.csv").then(data => {
        ...
        const totalOrders = new Set(data1.map(d => d["Mã đơn hàng"])).size;
        ...
        const aggregatedData = Array.from(
            ...
            ([key, values]) => ({
                ...
                "Mã đơn hàng": new Set(values.map(d => d["Mã đơn hàng"])).size
            })
        ).map(d => ({
            ...d,
            "Xác suất bán": (d["Mã đơn hàng"] / totalOrders) * 100, // Xác suất bán %
            "SL Đơn Bán": d["Mã đơn hàng"]
        }));
