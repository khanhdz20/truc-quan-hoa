document.addEventListener("DOMContentLoaded", function () {
    d3.csv("data.csv").then(data => {
        ...
        // Chuyển đổi dữ liệu và kiểm tra lỗi
        data.forEach(d => {
        ...
        const y = d3.scaleLinear()
            .domain([0, d3.max(aggregatedData, d => d["Doanh số bán TB"]) || 1]) // Tránh lỗi domain trống
            .nice()
            .range([height, 0]);
        ...
        const bars = chart.selectAll(".bar")
            ...
            .on("click", function (event, d) {
                const isFaded = d3.select(this).attr("opacity") === "0.3";
                bars.attr("opacity", 0.3);
                d3.select(this).attr("opacity", isFaded ? 1 : 0.3);
            });
